...
k = 10000            #Valores que se tienen que variar para la experimentacion.
n = 500000
duracion = 50
...
for experimento in range(3):
    for paso in range(duracion):
        ...
        porcentaje = 0;
        for cum in cumulos:
            porcentaje = porcentaje if cum < c else porcentaje + cum/n       #Si el cumulo es mas grande que el tamaño critico c, sumamos ese porcentaje

        print('Experimento:', experimento + 1, 'K:  ', k, '  n: ',  n, '  paso :',paso +1, '  C:', c, '  %Filtrado:  ', porcentaje)

    plt.hist(cumulos, bins=cortes, align='right', density=True)
    plt.xlabel('Tamaño')
    plt.ylabel('Frecuencia relativa')
    plt.ylim(0, 0.05)
    plt.title('Paso {:d} con ambos fenómenos'.format(experimento + 1))
    plt.savefig('p8p_ct' + format(experimento, '0{:d}'.format(digitos)) + '.png')
    plt.close()